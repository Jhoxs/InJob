...
indexCtrl.inicio = async(req, res) => {   
    try {
        ...
        let empVal = await pool.query('SELECT e.*, u.nombre, u.apellido,  SUM(ce.calificacion) AS cal FROM cal_empresa AS ce,usuario AS u, empleos AS e WHERE ce.id_empresa = e.id_empresa AND e.id_empresa = u.cedula GROUP BY (ce.id_empresa) ORDER BY (cal) DESC LIMIT 4');
        if(empVal.length == 0){
            empVal = 0;
        }
